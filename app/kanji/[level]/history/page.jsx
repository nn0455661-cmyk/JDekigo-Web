...
export default function KanjiHistoryPage() {
    ...
    useEffect(() => {
        if (rawLevel !== level) {
            router.replace(`/kanji/${level}/history`);
            return;
        }
        ...
        const loadHistory = async () => {
            ...
            if (!user) {
                setHistoryItems(loadLocalTestHistory("kanji", level));
                setLoading(false);
                return;
            }
            try {
                const res = await fetchTestHistory("kanji", level);
        ...
        <section className="dashboard-shell space-y-5 p-4 sm:p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="section-title">{t("kanji.historyTitle", "Lịch sử làm bài")} - {level}</h1>
                        ...
                        >
                            <div className="min-w-0">
                                <p className="truncate text-sm font-semibold text-[var(--color-text)]">
                                    {item.title || item.testTitle || t("kanji.defaultReviewTitle", "Ôn tập")}
                                </p>
                                <p className="mt-0.5 text-xs text-[var(--color-text-soft)]">
                                    {t("kanji.historyDateLabel", "Ngày làm")}: {formatDateTime(item.createdAt, locale)}
                                    {" | "}
                                    {t("kanji.historyDurationLabel", "Thời gian làm")}: {formatDuration(item.durationSeconds)}
                                </p>
                            </div>
