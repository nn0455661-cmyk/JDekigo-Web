...
export default function VocabularyTestListPage() {
    const { t } = useLanguage();
    ...
    useEffect(() => {
        if (rawLevel !== level) {
            router.replace(`/vocabulary/${level}/test`);
            return;
        }
        ...
        <section className="dashboard-shell space-y-5 p-4 sm:p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="section-title">{t("vocabulary.testSetupTitle", "Đề kiểm tra từ vựng")} - {level}</h1>
                        ...
                        >
                            <div className="flex items-center justify-between gap-3">
                                <p className="inline-flex min-w-0 items-center gap-2 text-lg font-bold text-[var(--color-text)]">
                                    <FileText className="h-4 w-4 shrink-0 text-[var(--icon-vocabulary)]" />
